...
const table = new Fliplet.UI.Table({
    ...
    columns: [
        {
            ...
            field: 'name',
            render: function(row) {
                return `${row.name} <i data-expand class="fas fa-chevron-right expand-chevron"></i>`;
            }
        },
        {
            name: 'Status',
            field: 'status',
            render: function(row) {
                // Status badges that are clickable to expand
                return `<span data-expand class="status-badge clickable-badge">${row.status}</span>`;
            }
        },
        {
            name: 'Actions',
            render: function(row) {
                return `
                    ...
                    <button data-expand class="btn-details">View Details</button>
                `;
            }
        }
    ],
    expandable: {
        ...
        onExpand: function(row) {
            return generateDetailedView(row);
        }
    },
    data: userData
});
...
table.on('cell:interaction', function(detail) {
    ...
    } else if (trigger.classList.contains('btn-details')) {
...
const table = new Fliplet.UI.Table({
            ...
            render: function(row) {
                ...
                return `
                    <div class="user-cell">
                        ...
                        <small data-expand class="expand-trigger">
                            ${icon} ${expandText} Details
                        </small>
                    </div>
                `;
            }
    ...
    expandable: {
        enabled: true,
        onExpand: function(row) {
            return `<div>Details for ${row.name}</div>`;
        }
    },
    data: userData
});

// Update trigger text after expansion/collapse
table.on('expand:complete', function(detail) {
    updateTriggerText(detail.rowEl, true);
});

table.on('collapse:complete', function(detail) {
    updateTriggerText(detail.rowEl, false);
});

function updateTriggerText(rowEl, isExpanded) {
    const trigger = rowEl.querySelector('.expand-trigger');
    if (trigger) {
        const icon = isExpanded ? '🔽' : '▶️';
        const text = isExpanded ? 'Hide' : 'Show';
        trigger.innerHTML = `${icon} ${text} Details`;
    }
}
```
